...
2. **Build the artifact:** `./build-app.sh`. This writes `dist/Murmur.zip`, `dist/Murmur.dmg`, and `dist/Murmur.app`. Confirm both `dist/Murmur.zip` and `dist/Murmur.dmg` exist after.
...
6. **Publish release:** `gh release create "v$ARGUMENTS" --latest --title "Murmur v$ARGUMENTS" --notes "<short release notes>" dist/Murmur.zip dist/Murmur.dmg`. Draft the notes from the diff since the previous tag — `git log --oneline "$(git describe --tags --abbrev=0 HEAD^)..HEAD"` gives the commit list. Keep notes to 3–6 bullet points.
...
- **Both assets are load-bearing:** `Murmur.zip` feeds the in-app self-updater (UpdateChecker), and `Murmur.dmg` is what murmur.ketok.id's download button fetches via `releases/latest/download/Murmur.dmg` — drop it and the website 404s (this happened across v2026.06.11.0–.4).
